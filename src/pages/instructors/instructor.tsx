...
const User: React.FC = () => {
    ...
    const getInstructors = useCallback(async () => {
        ...
        try {
            ...
            if (response && response.data) {
                setInstructor(response.data.instructors.instructors);
                // console.log(response.data);
                localStorage.setItem('instructor_lenght', (response.data.instructors.instructors.length).toString());
            } else {
    ...
    const handleUserCreate = async (e: FormEvent) => {
        ...
        try {
            ...
            const response = await InstructorService.signInFn(firstname, lastname, email, phoneNumber, password, profileImageUrl);
            if (response && response.data) {
                console.log(response.data);
                    ...
                    <>
                            ...
                            <div className="col-12 col-md-8 col-lg-12 mb-4">
                                <div className="card h-100 shadow-sm hover-shadow transition">
                                    <div className="card-body d-flex flex-column">
                                        <h5 className="card-title text-primary">Current instructors</h5>
                        ...
                        <table className="mt-5 table table-striped">
                            ...
                            <tbody className="align-content-center align-content-center">
                                {Array.isArray(instructors) && instructors.map((user, index) => (
                                    <tr key={index} onClick={() => navigate(`/instructor/${user._id}`)}>
                                        <td className="">
